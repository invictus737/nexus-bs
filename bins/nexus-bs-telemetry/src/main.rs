...
//! nexus-bs-telemetry — minimal WebSocket telemetry receiver
//!
...
#[derive(Parser)]
#[command(name = "nexus-bs-telemetry", version, about = "Nexus-BS TETRA telemetry service")]
struct Args {
        ...
        let name = if let Some(db) = auth_db {
            match check_basic_auth(req, db) {
                ...
                None => {
                    ...
                    err.headers_mut()
                        .insert("WWW-Authenticate", "Basic realm=\"nexus-bs-telemetry\"".parse().unwrap());
                    return Err(err);
                }
            }
        } else {
