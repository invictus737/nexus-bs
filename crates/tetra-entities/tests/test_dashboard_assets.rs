...
fn external_dashboard_asset_manifest_is_coherent() {
    ...
    assert!(
        index.contains(r#"id="easyStartModal""#)
            ...
            && dashboard_server.contains("serve_service_shutdown")
            && dashboard_server.contains("request_host_poweroff")
            && dashboard_server.contains("local_poweroff_accepted")
    ...
    assert!(
        dashboard_server.contains(r#"run_update_command_privileged(&update, "apt-get", &["install", "-y", deb_path_str])"#)
            ...
            && deb_postinst.contains("systemctl enable $enable_after_legacy_replace")
