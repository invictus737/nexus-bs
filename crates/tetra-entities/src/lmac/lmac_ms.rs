...
use tetra_core::{BurstType, PhyBlockNum, PhyBlockType, PhysicalChannel, Sap, TdmaTime, TrainingSequence, unimplemented_log};
...
impl LmacMs {
    ...
    fn rx_tmv_unitdata_req_slot(&mut self, queue: &mut MessageQueue, mut prim: TmvUnitdataReqSlot) {
        if prim.ul_phy_chan != PhysicalChannel::Cp {
            tracing::warn!(
                "LMAC-MS: unsupported uplink physical channel {:?} for SCH/HU transmit request, dropping",
                prim.ul_phy_chan
            );
            return;
        }

        if prim.bbk.is_some() || prim.blk2.is_some() {
            tracing::warn!("LMAC-MS: SCH/HU uplink request with BBK or blk2 is unsupported, dropping");
            return;
        }

        let Some(blk1) = prim.blk1.take() else {
            tracing::warn!("LMAC-MS: SCH/HU uplink request missing blk1, dropping");
            return;
        };

        if blk1.logical_channel != LogicalChannel::SchHu {
            tracing::warn!(
                "LMAC-MS: unsupported uplink logical channel {:?}; only SCH/HU random-access signalling is implemented",
                blk1.logical_channel
            );
            return;
        }

        // EN 300 392-2 clauses 8.3.1.4.3, 9.4.4.3.3, 9.5.3 and
        // 23.5.2.4: SCH/HU carries one 92-bit type-1 uplink signalling
    ...
    fn rx_tmv_prim(&mut self, queue: &mut MessageQueue, message: SapMsg) {
        ...
        match message.msg {
            ...
            SapMsgInner::TmvUnitdataReq(prim) => {
                self.rx_tmv_unitdata_req_slot(queue, prim);
            }
            _ => {
                tracing::error!("BUG: unexpected message or state -- routing error");
                return;
