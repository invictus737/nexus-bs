...
const N203_CONTROL_CHANNEL_SLOTS_WITHOUT_FRAGMENT: i32 = 4;
const DEFRAG_TS_BEFORE_TIMEOUT: i32 = N203_CONTROL_CHANNEL_SLOTS_WITHOUT_FRAGMENT * 4;
...
mod tests {
    ...
    #[test]
    fn test_n203_discards_incomplete_fragment() {
        debug::setup_logging_verbose();
        ...
        // EN 300 392-2 clause 23.4.3.1.1: the MS-MAC discards a partially
        // reconstructed downlink TM-SDU after N.203 consecutive control-channel
        // slots without a fragment. Annex B default N.203 is >= 4; this stack
        // uses the minimum default until network-specific configuration exists.
        defragger.insert_last(&mut stale_end, t.add_timeslots(DEFRAG_TS_BEFORE_TIMEOUT + 4));
        assert!(
            defragger
                .take_defragged_buf(t.add_timeslots(DEFRAG_TS_BEFORE_TIMEOUT + 4))
                .is_none()
        );
    }
}
