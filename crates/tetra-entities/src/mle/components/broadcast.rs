...
impl MleBroadcast {
    ...
    pub fn send_broadcast(&mut self, queue: &mut MessageQueue, req_handle: i32) -> bool {
        let broadcast_type = self.determine_next_broadcast_type();
        self.last_broadcast_type = broadcast_type;

        match broadcast_type {
            BroadcastType::NetworkTime => self.send_d_nwrk_broadcast(queue, req_handle),
            BroadcastType::None => false,
        }
    }
    ...
    fn send_d_nwrk_broadcast(&self, queue: &mut MessageQueue, req_handle: i32) -> bool {
        ...
        let sent = if has_neighbours {
            ...
            // Time only, no neighbours: keep the legacy on-air field semantics
            // for maximum MS compatibility, with number_of_ca_neighbour_cells = Some(0).
            ...
            self.transmit_pdu(queue, pdu_time_only, "time-only", req_handle)
        } else {
            false
        };
    ...
    fn transmit_pdu(&self, queue: &mut MessageQueue, pdu: DNwrkBroadcast, label: &str, req_handle: i32) -> bool {
        ...
        let sapmsg = SapMsg {
            ...
            msg: SapMsgInner::TlaTlUnitdataReqBl(TlaTlUnitdataReqBl {
                ...
                subscriber_class: self.config.config().cell.subscriber_class as i32,
                fcs_flag: false,
