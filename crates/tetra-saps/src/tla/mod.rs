...
/// Local TLA-SAP report values. EN 300 392-2 defines these reports as SAP
/// semantics, not as over-air encodings.
pub const TLA_REPORT_NO_SPECIFIC_REPORT: Todo = 0;
pub const TLA_REPORT_FIRST_COMPLETE_TRANSMISSION: Todo = 1;
pub const TLA_REPORT_FAILED_TRANSFER: Todo = 2;
pub const TLA_REPORT_SUCCESSFUL_TRANSFER: Todo = 3;
...
pub struct TlaTlUnitdataReqBl {
    ...
    pub fcs_flag: bool,
    pub air_interface_encryption: Option<Todo>,
    /// EN 300 392-2 clause 18.3.5.3.1 passes the layer-3 stealing repeats
    /// parameter through MLE/LLC for layer-2 scheduling.
    pub stealing_repeats_flag: Option<bool>,
    // pub data_prio: Todo,
    pub packet_data_flag: bool,
    /// EN 300 392-2 clause 22.3.2.4.1 note 1: `None` means the service user
    /// did not indicate N.253, so the LLC designer value applies. `Some(0)` is
    /// an explicit no-repetition request and must remain distinguishable.
    pub n_tlsdu_repeats: Option<u8>,
