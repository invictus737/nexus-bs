...
impl DConnect {
    fn validate(&self) -> Result<(), PduParseErr> {
        // EN 300 392-2 clause 14.7.1.4 / table 14.8 define fixed-width
        // Type1/Type2 fields. Reject over-wide values instead of silently
        // truncating them when writing the bitstream.
        ...
        if let Some(call_priority) = self.call_priority {
            if call_priority > MAX_U4 {
                return Err(PduParseErr::InvalidValue {
                    field: "call_priority",
                    value: call_priority,
                });
...
mod tests {
    ...
    fn minimal_d_connect(call_identifier: u16) -> DConnect {
        DConnect {
            call_identifier,
    ...
    #[test]
    fn d_connect_rejects_overwide_call_identifier() {
        let pdu = minimal_d_connect(0x4000);
        let mut buffer = BitBuffer::new_autoexpand(40);

        // EN 300 392-2 clause 14.7.1.4: D-CONNECT call identifier is 14 bits.
        assert_eq!(
    ...
    #[test]
    fn d_connect_rejects_overwide_call_priority() {
        let mut pdu = minimal_d_connect(1);
        pdu.call_priority = Some(0x10);
        let mut buffer = BitBuffer::new_autoexpand(40);

        // EN 300 392-2 table 14.8: call priority is 4 bits when present.
        assert_eq!(
            pdu.to_bitbuf(&mut buffer),
            Err(PduParseErr::InvalidValue {
                field: "call_priority",
    ...
    #[test]
    fn d_connect_rejects_overwide_temporary_address() {
        let mut pdu = minimal_d_connect(1);
        pdu.temporary_address = Some(0x0100_0000);
        let mut buffer = BitBuffer::new_autoexpand(64);

        // EN 300 392-2 table 14.8: temporary address is 24 bits when present.
        assert_eq!(
            pdu.to_bitbuf(&mut buffer),
            Err(PduParseErr::InvalidValue {
                field: "temporary_address",
    ...
    #[test]
    fn d_connect_rejects_overwide_notification_indicator() {
        let mut pdu = minimal_d_connect(1);
        pdu.notification_indicator = Some(0x40);
        let mut buffer = BitBuffer::new_autoexpand(40);

        // EN 300 392-2 table 14.8: notification indicator is 6 bits.
        assert_eq!(
            pdu.to_bitbuf(&mut buffer),
            Err(PduParseErr::InvalidValue {
                field: "notification_indicator",
