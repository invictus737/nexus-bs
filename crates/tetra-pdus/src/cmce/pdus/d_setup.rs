...
impl DSetup {
    fn validate(&self) -> Result<(), PduParseErr> {
        // EN 300 392-2 clause 14.7.1.12 / table 14.15 fixes the host-visible
        // field widths serialized below. Reject over-wide values before any
        // bits are written so callers get a recoverable PDU error.
        ...
        if let Some(ssi) = self.calling_party_address_ssi {
            if ssi as u64 > MAX_U24 {
                return Err(PduParseErr::InvalidValue {
        ...
        if let Some(extension) = self.calling_party_extension {
            if extension as u64 > MAX_U24 {
                return Err(PduParseErr::InvalidValue {
        ...
        if self.calling_party_extension.is_some() && self.calling_party_address_ssi.is_none() {
            return Err(PduParseErr::Inconsistency {
                field: "calling_party_extension",
                reason: "not valid without calling_party_address_ssi",
            });
        }
...
mod tests {
    ...
    fn base_d_setup() -> DSetup {
        DSetup {
            ...
            temporary_address: Some(0x0012_3456),
    ...
    fn serialize_err(pdu: &DSetup) -> PduParseErr {
    ...
    #[test]
    fn d_setup_rejects_overwide_call_identifier() {
        let pdu = DSetup {
    ...
    #[test]
    fn d_setup_rejects_overwide_call_priority() {
        let pdu = DSetup {
            call_priority: 0x10,
            ..base_d_setup()
        };
    ...
    #[test]
    fn d_setup_rejects_overwide_notification_indicator() {
        let pdu = DSetup {
    ...
    #[test]
    fn d_setup_rejects_overwide_temporary_address() {
        let pdu = DSetup {
    ...
    #[test]
    fn d_setup_rejects_overwide_calling_party_address_ssi() {
        let pdu = DSetup {
    ...
    #[test]
    fn d_setup_rejects_overwide_calling_party_extension() {
        let pdu = DSetup {
    ...
    #[test]
    fn d_setup_rejects_calling_party_extension_without_ssi() {
        let pdu = DSetup {
            calling_party_address_ssi: None,
            calling_party_extension: Some(0x0012_3456),
            ..base_d_setup()
        };
