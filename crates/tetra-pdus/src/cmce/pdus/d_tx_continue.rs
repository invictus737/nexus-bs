...
impl DTxContinue {
    fn validate(&self) -> Result<(), PduParseErr> {
        // EN 300 392-2 clause 14.7.1.14 / table 14.17 fixes the Call
...
mod tests {
    ...
    fn base_d_tx_continue() -> DTxContinue {
        DTxContinue {
    ...
    fn serialize_err(pdu: &DTxContinue) -> PduParseErr {
        let mut buf = BitBuffer::new_autoexpand(64);
        pdu.to_bitbuf(&mut buf).unwrap_err()
    }

    #[test]
    fn d_tx_continue_rejects_overwide_call_identifier() {
        let pdu = DTxContinue {
    ...
    #[test]
    fn d_tx_continue_rejects_overwide_notification_indicator() {
        let pdu = DTxContinue {
