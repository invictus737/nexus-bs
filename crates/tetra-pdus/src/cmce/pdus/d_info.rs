...
impl DInfo {
    fn validate(&self) -> Result<(), PduParseErr> {
        // EN 300 392-2 clause 14.7.1.8 / table 14.11 fixes the bit width
        // for each Type 1 and Type 2 field below. Polling-related validity is
        // call-context dependent, so CMCE enforces that at routing/state level.
        ...
        validate_optional_width("new_call_identifier", self.new_call_identifier, MAX_U14)?;
        validate_optional_width("call_time_out", self.call_time_out, MAX_U4)?;
        validate_optional_width(
            "call_time_out_set_up_phase_t301_t302_",
            self.call_time_out_set_up_phase_t301_t302_,
            MAX_U3,
        )?;
        validate_optional_width("call_ownership", self.call_ownership, MAX_U1)?;
        validate_optional_width("modify", self.modify, MAX_U9)?;
        validate_optional_width("call_status", self.call_status, MAX_U3)?;
        validate_optional_width("temporary_address", self.temporary_address, MAX_U24)?;
        validate_optional_width("notification_indicator", self.notification_indicator, MAX_U6)?;
        validate_optional_width("poll_response_percentage", self.poll_response_percentage, MAX_U6)?;
        validate_optional_width("poll_response_number", self.poll_response_number, MAX_U6)?;
...
fn validate_optional_width(field: &'static str, value: Option<u64>, max: u64) -> Result<(), PduParseErr> {
...
mod tests {
    ...
    fn base_d_info() -> DInfo {
        DInfo {
            call_identifier: 0x1234,
            reset_call_time_out_timer_t310_: false,
            poll_request: false,
            new_call_identifier: Some(0x1234),
            call_time_out: Some(0x0f),
            call_time_out_set_up_phase_t301_t302_: Some(0x07),
            call_ownership: Some(0x01),
            modify: Some(0x01ff),
            call_status: Some(0x07),
            temporary_address: Some(0x00aa_bbcc),
            notification_indicator: Some(0x3f),
            poll_response_percentage: Some(0x3f),
            poll_response_number: Some(0x3f),
            dtmf: None,
            facility: None,
    ...
    fn serialize_err(pdu: &DInfo) -> PduParseErr {
        let mut buf = BitBuffer::new_autoexpand(128);
        pdu.to_bitbuf(&mut buf).unwrap_err()
    }

    #[test]
    fn d_info_rejects_overwide_call_identifier() {
        let pdu = DInfo {
    ...
    #[test]
    fn d_info_rejects_overwide_new_call_identifier() {
        let pdu = DInfo {
    ...
    #[test]
    fn d_info_rejects_overwide_call_time_out() {
        let pdu = DInfo {
    ...
    #[test]
    fn d_info_rejects_overwide_setup_phase_timeout() {
        let pdu = DInfo {
            call_time_out_set_up_phase_t301_t302_: Some(0x08),
            ..base_d_info()
        };
        ...
        assert!(matches!(
            serialize_err(&pdu),
            PduParseErr::InvalidValue {
                field: "call_time_out_set_up_phase_t301_t302_",
                value: 0x08
            }
        ));
    }
    ...
    #[test]
    fn d_info_rejects_overwide_call_ownership() {
        let pdu = DInfo {
            call_ownership: Some(0x02),
            ..base_d_info()
        };
    ...
    #[test]
    fn d_info_rejects_overwide_modify() {
        let pdu = DInfo {
            modify: Some(0x0200),
            ..base_d_info()
        };
    ...
    #[test]
    fn d_info_rejects_overwide_call_status() {
        let pdu = DInfo {
    ...
    #[test]
    fn d_info_rejects_overwide_temporary_address() {
        let pdu = DInfo {
            temporary_address: Some(0x0100_0000),
            ..base_d_info()
        };
    ...
    #[test]
    fn d_info_rejects_overwide_notification_indicator() {
        let pdu = DInfo {
    ...
    #[test]
    fn d_info_rejects_overwide_poll_response_percentage() {
        let pdu = DInfo {
            poll_response_percentage: Some(0x40),
            ..base_d_info()
        };
        ...
        assert!(matches!(
            serialize_err(&pdu),
            PduParseErr::InvalidValue {
                field: "poll_response_percentage",
                value: 0x40
            }
        ));
    }
    ...
    #[test]
    fn d_info_rejects_overwide_poll_response_number() {
        let pdu = DInfo {
            poll_response_number: Some(0x40),
            ..base_d_info()
        };
