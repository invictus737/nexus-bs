...
impl DTxGranted {
    fn validate(&self) -> Result<(), PduParseErr> {
        // EN 300 392-2 table 14.18 note 1: reserved bit shall be set to 0.
        if self.reserved {
        ...
        // EN 300 392-2 table 14.18 note 2 and table 14.82: TPTI controls
...
mod tests {
    ...
    fn base_d_tx_granted() -> DTxGranted {
        DTxGranted {
    ...
    fn round_trip(pdu: &DTxGranted) -> (DTxGranted, usize) {
        let mut buf = BitBuffer::new_autoexpand(128);
        pdu.to_bitbuf(&mut buf).expect("serialize D-TX GRANTED");
        let len = buf.get_len();
        buf.seek(0);
        let decoded = DTxGranted::from_bitbuf(&mut buf).expect("parse D-TX GRANTED");
        (decoded, len)
    }

    #[test]
    fn d_tx_granted_roundtrips_tpti_ssi_without_address_pbits() {
        let pdu = base_d_tx_granted();
    ...
    #[test]
    fn d_tx_granted_roundtrips_tpti_tsi_without_address_pbits() {
        let pdu = DTxGranted {
    ...
    #[test]
    fn d_tx_granted_parser_rejects_reserved_bit_set() {
        let mut buf = BitBuffer::new_autoexpand(32);
        buf.write_bits(CmcePduTypeDl::DTxGranted.into_raw(), 5);
        buf.write_bits(1, 14);
        buf.write_bits(0, 2);
        buf.write_bits(0, 1);
        buf.write_bits(0, 1);
        ...
        assert_eq!(
            DTxGranted::from_bitbuf(&mut buf).unwrap_err(),
    ...
    #[test]
    fn d_tx_granted_rejects_reserved_bit_on_serialize() {
        let pdu = DTxGranted {
            reserved: true,
            ..base_d_tx_granted()
        };
    ...
    #[test]
    fn d_tx_granted_rejects_reserved_tpti_values() {
        for value in [0, 3] {
            let pdu = DTxGranted {
    ...
    #[test]
    fn d_tx_granted_requires_address_for_tpti_ssi() {
        let pdu = DTxGranted {
            transmitting_party_address_ssi: None,
            ..base_d_tx_granted()
        };
    ...
    #[test]
    fn d_tx_granted_requires_extension_for_tpti_tsi() {
        let pdu = DTxGranted {
            transmitting_party_type_identifier: Some(TPTI_TSI),
            transmitting_party_address_ssi: Some(0x00aa_bbcc),
            transmitting_party_extension: None,
    ...
    #[test]
    fn d_tx_granted_rejects_address_fields_without_tpti() {
        for pdu in [
            DTxGranted {
    ...
    #[test]
    fn d_tx_granted_rejects_tpti_tsi_without_extension_on_parse() {
