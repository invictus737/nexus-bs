...
mod tests {
    ...
    fn parser_rejects_wrong_mac_pdu_type_without_panic() {
        let mut buf = BitBuffer::from_bitstr("010");

        assert_eq!(
            MacUSignal::from_bitbuf(&mut buf).unwrap_err(),
    ...
    #[test]
    fn serializer_round_trips_second_half_stolen_flag() {
        let pdu = MacUSignal { second_half_stolen: true };
        let mut buf = BitBuffer::new(3);

        pdu.to_bitbuf(&mut buf);
        buf.seek(0);
        let parsed = MacUSignal::from_bitbuf(&mut buf).unwrap();

        assert!(parsed.second_half_stolen);
    }
}
