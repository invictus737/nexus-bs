...
use crate::mm::enums::location_update_accept_type::LocationUpdateAcceptType;
...
impl DLocationUpdateAccept {
    fn validate_type2_width(field: &'static str, value: Option<u64>, bits: u8) -> Result<(), PduParseErr> {
    ...
    fn validate_for_serialization(&self) -> Result<(), PduParseErr> {
        // EN 300 392-2 clause 16.9.2.7 table 16.11 fixes these Type-2
        // information element widths. Rejecting too-wide values here prevents
        // locally generated D-LOCATION UPDATE ACCEPT PDUs from silently
        // truncating attach identity, class, or SCCH/frame-18 information.
        Self::validate_type2_width("ssi", self.ssi, 24)?;
        Self::validate_type2_width("address_extension", self.address_extension, 24)?;
        Self::validate_type2_width("subscriber_class", self.subscriber_class, 16)?;
        Self::validate_type2_width(
    ...
    pub fn from_bitbuf(buffer: &mut BitBuffer) -> Result<Self, PduParseErr> {
        ...
        let result = LocationUpdateAcceptType::try_from(val);
    ...
    pub fn to_bitbuf(&self, buffer: &mut BitBuffer) -> Result<(), PduParseErr> {
        ...
        // Type1
        buffer.write_bits(self.location_update_accept_type.into_raw(), 3);
...
mod tests {
    ...
    #[test]
    fn test_d_location_update_accept_raw_one_is_temporary_registration() {
        // EN 300 392-2 clause 16.10.35a table 16.68 defines DL accept raw
        // value 001 as Temporary registration, not the UL migrating update.
        let test_vec = "01010010";
        let mut buf_in = BitBuffer::from_bitstr(test_vec);
        let pdu = DLocationUpdateAccept::from_bitbuf(&mut buf_in).expect("Failed parsing");

        assert_eq!(pdu.location_update_accept_type, LocationUpdateAcceptType::TemporaryRegistration);
    ...
    #[test]
    fn test_d_location_update_accept_rejects_type2_values_that_exceed_etsi_widths() {
        // EN 300 392-2 clause 16.9.2.7 table 16.11 defines SSI/MNI as
        // 24-bit fields, subscriber class as 16 bits, and SCCH information and
        // distribution on 18th frame as 6 bits.
        for (field, pdu) in [
            (
                "ssi",
                DLocationUpdateAccept {
            ...
            (
                "subscriber_class",
                DLocationUpdateAccept {
                    subscriber_class: Some(0x1_0000),
                    ..base_accept()
                },
            ),
            (
                "scch_information_and_distribution_on_18th_frame",
                DLocationUpdateAccept {
                    scch_information_and_distribution_on_18th_frame: Some(0x40),
                    ..base_accept()
                },
            ),
        ] {
            ...
            assert_eq!(
                ...
                Err(PduParseErr::InvalidValue {
                    field,
                    value: match field {
                        "ssi" | "address_extension" => 0x0100_0000,
                        "subscriber_class" => 0x1_0000,
                        "scch_information_and_distribution_on_18th_frame" => 0x40,
                        _ => unreachable!(),
                    },
                })
            );
            assert_eq!(out.get_len(), 0);
        }
    }

    #[test]
    fn test_d_location_update_accept_serializes_generic_type4_when_payload_fits() {
        // Annex E.2.2 permits D-LOCATION UPDATE ACCEPT optional Type-4
        // elements. Generic Type-4 reserialization is safe when the parser has
        // retained the full payload in Type4FieldGeneric::data.
        let pdu = DLocationUpdateAccept {
            new_registered_area: Some(Type4FieldGeneric {
                field_id: MmType34ElemIdDl::NewRegisteredArea.into(),
                len: 12,
                elems: 1,
                data: 0x0abc,
            }),
        ...
        pdu.to_bitbuf(&mut out).expect("short generic Type4 should serialize");
        let bitstr = out.to_bitstr();
        let mut input = BitBuffer::from_bitstr(&bitstr);
        let parsed = DLocationUpdateAccept::from_bitbuf(&mut input).expect("serialized PDU should parse");

        assert_eq!(
            parsed.new_registered_area,
            Some(Type4FieldGeneric {
                field_id: MmType34ElemIdDl::NewRegisteredArea.into(),
                len: 12,
                elems: 1,
                data: 0x0abc,
        ...
        assert_eq!(input.get_len_remaining(), 0);
    }

    fn base_accept() -> DLocationUpdateAccept {
        DLocationUpdateAccept {
            location_update_accept_type: LocationUpdateAcceptType::ItsiAttach,
            ssi: None,
