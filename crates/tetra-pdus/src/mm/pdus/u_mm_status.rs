...
impl UMmStatus {
    ...
    pub fn to_bitbuf(&self, buffer: &mut BitBuffer) -> Result<(), PduParseErr> {
        ...
        match (
            ...
            self.status_uplink_dependent_information_len,
        ) {
            (Some(value), Some(len)) if len <= 64 && (len == 64 || value >> len == 0) => {
                buffer.write_bits(value, len);
            }
            (Some(_), Some(len)) if len > 64 => {
                return Err(PduParseErr::InvalidValue {
                    field: "status_uplink_dependent_information_len",
                    value: len as u64,
                });
            }
            (Some(value), Some(_)) => {
                return Err(PduParseErr::InvalidValue {
                    field: "status_uplink_dependent_information",
                    value,
                });
            }
            (Some(_), None) => {
            ...
            (None, Some(_)) => {
                return Err(PduParseErr::Inconsistency {
                    field: "status_uplink_dependent_information_len",
                    reason: "present without status_uplink_dependent_information",
                });
            }
            (None, None) => {}
        }
...
mod tests {
    ...
    #[test]
    fn test_u_mm_status_rejects_missing_dependent_information_len() {
        let pdu = UMmStatus {
            status_uplink: StatusUplink::ChangeOfEnergySavingModeRequest,
            status_uplink_dependent_information: Some(1),
            status_uplink_dependent_information_len: None,
        };
    ...
    #[test]
    fn test_u_mm_status_rejects_dependent_information_len_without_value() {
        let pdu = UMmStatus {
            status_uplink: StatusUplink::ChangeOfEnergySavingModeRequest,
            status_uplink_dependent_information: None,
            status_uplink_dependent_information_len: Some(3),
        };
        let mut buf = BitBuffer::new_autoexpand(16);
    ...
    #[test]
    fn test_u_mm_status_rejects_dependent_information_value_that_exceeds_len() {
        let pdu = UMmStatus {
            status_uplink: StatusUplink::ChangeOfEnergySavingModeRequest,
            status_uplink_dependent_information: Some(0b1000),
            status_uplink_dependent_information_len: Some(3),
        };
        let mut buf = BitBuffer::new_autoexpand(16);
