...
use crate::mm::pdus::attach_detach_group_identity_validation::{
    validate_group_identity_uplink_collection, validate_group_report_response, validate_type3_generic_field,
};
...
impl UAttachDetachGroupIdentity {
    fn validate(&self) -> Result<(), PduParseErr> {
        validate_group_report_response(&self.group_report_response, MmType34ElemIdUl::GroupReportResponse.into_raw())?;
...
mod tests {
    ...
    #[test]
    fn u_attach_detach_group_identity_rejects_bad_group_report_response_length() {
        let pdu = UAttachDetachGroupIdentity {
        ...
        assert_eq!(
            ...
            Err(PduParseErr::InconsistentLength { expected: 1, found: 2 })
        );
        assert_eq!(buf.get_len(), 0);
    }

    #[test]
    fn u_attach_detach_group_identity_rejects_empty_uplink_collection() {
        let pdu = UAttachDetachGroupIdentity {
            group_identity_report: false,
            group_identity_attach_detach_mode: false,
    ...
    #[test]
    fn u_attach_detach_group_identity_rejects_empty_uplink_collection_on_parse() {
        let mut buf = BitBuffer::new_autoexpand(32);
        buf.write_bits(MmPduTypeUl::UAttachDetachGroupIdentity.into_raw(), 4);
        buf.write_bits(0, 1);
        buf.write_bits(0, 1);
        delimiters::write_obit(&mut buf, 1);
        typed::write_type34_header_generic(&mut buf, MmType34ElemIdUl::GroupIdentityUplink.into_raw());
        buf.write_bits(6, 11);
        buf.write_bits(0, 6);
        ...
        assert_eq!(
            UAttachDetachGroupIdentity::from_bitbuf(&mut buf).unwrap_err(),
