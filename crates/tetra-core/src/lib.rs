...
/// Full stack version string, e.g. "v0.1.72-g2aad62c"
...
mod tests {
    ...
    fn product_identity_tracks_workspace_version() {
        ...
        assert_eq!(PRODUCT_BASE_VERSION, "0.1.72");
        assert_eq!(PRODUCT_VERSION_SUFFIX, "");
        assert_eq!(PRODUCT_VERSION, "0.1.72");
        assert_eq!(PRODUCT_VERSION_TAG, "v0.1.72");
        assert_eq!(PRODUCT_USER_AGENT, "Nexus-BS/v0.1.72");
        assert_eq!(CONTROL_PROTOCOL_VERSION, "nexus-bs-control-v0.1.72");
        assert_eq!(TELEMETRY_PROTOCOL_VERSION, "nexus-bs-telemetry-v0.1.72");
    }
}
