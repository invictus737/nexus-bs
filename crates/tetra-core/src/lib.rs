...
mod tests {
    ...
    fn product_identity_tracks_workspace_version() {
        ...
        assert_eq!(PRODUCT_BASE_VERSION, version);
        assert_eq!(PRODUCT_VERSION_SUFFIX, "");
        assert_eq!(PRODUCT_VERSION, version);
        assert_eq!(PRODUCT_VERSION_TAG, tag);
        assert_eq!(PRODUCT_USER_AGENT, format!("Nexus-BS/{}", tag));
        assert_eq!(CONTROL_PROTOCOL_VERSION, format!("nexus-bs-control-{}", tag));
        assert_eq!(TELEMETRY_PROTOCOL_VERSION, format!("nexus-bs-telemetry-{}", tag));
    }
}
