...
validate_tag() {
    local version tag tag_commit head_commit
    version="$1"
    ...
    if [ "${GITHUB_REF_TYPE:-}" = "tag" ] || [ -n "${RELEASE_TAG:-}" ]; then
        ...
        tag_commit="$(git rev-list -n 1 "$tag")" || die "could not resolve release tag '${tag}'"
        head_commit="$(git rev-parse HEAD)" || die "could not resolve checkout HEAD"
        [ "$head_commit" = "$tag_commit" ] || die "checkout HEAD ${head_commit} does not match ${tag} commit ${tag_commit}"
        ...
        RELEASE_BUILD_HASH="$(git rev-parse --short=8 "$tag_commit")"
        NEXUS_BS_BUILD_HASH="$RELEASE_BUILD_HASH"
        export RELEASE_COMMIT RELEASE_BUILD_HASH NEXUS_BS_BUILD_HASH
    else
        tag="v${version}"
    fi
    RELEASE_TAG="$tag"
    ...
    if [ -n "${RELEASE_BUILD_HASH:-}" ]; then
        log "Release commit ${RELEASE_COMMIT}"
        log "Release build hash ${RELEASE_BUILD_HASH}"
    fi
}
...
sync_distribution_payload() {
    ...
    install -m 0755 scripts/nexus-bs-factory-reset-clean "$DIST_DIR/scripts/nexus-bs-factory-reset-clean"
...
update_distribution_readme() {
    ...
    (
        ...
        sha256_cmd \
            ...
            scripts/nexus-bs-factory-reset-clean \
...
- `scripts/`: service and factory-reset helpers.
...
verify_distribution() {
    local version name dirty_matches stack_version
    version="$1"
    ...
    if [ -n "${RELEASE_BUILD_HASH:-}" ]; then
        stack_version="v${version}-${RELEASE_BUILD_HASH}"
        for name in "${BIN_NAMES[@]}"; do
            strings "$DIST_DIR/bin/$name" | grep -F "$stack_version" >/dev/null \
                || die "release binary ${name} does not contain tag build string '${stack_version}'"
        done
    fi
    for name in "${BIN_NAMES[@]}"; do
